const abmCtrl = {};
const User = require("../../config/models/mongo/User");
...
abmCtrl.loguedUser = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "User profile with all the data", body: userData });
  } catch (error) {
...
abmCtrl.usedAvatars = async (req, res) => {
  ...
  const avatars = data.map(({ userAvatar, userImage }) => ({
    userAvatar,
    userImage,
  }));
  const message = {
    message: "This are the existing avatars in the db",
    body: avatars,
  };
  res.json(message);
};
