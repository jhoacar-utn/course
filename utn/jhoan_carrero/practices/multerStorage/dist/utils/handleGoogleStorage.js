...
const {
  storageGoogle: configStorage
} = require("../config/storage");

const {
  getFileName
} = require("./handleFileName");

const customHandleFile = function (req, file, cb) {
  const storage = new Storage(); // A bucket is a container for objects (files).

  const bucket = storage.bucket(configStorage.bucketName);
  if (!bucket) return cb("bucket is a required field.");
